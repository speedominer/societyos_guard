...
class ApprovalResultScreen extends StatelessWidget {
  ...
  const ApprovalResultScreen(
      {super.key, required this.approved, required this.visitorId, this.event});

  Future<void> _recordEntry(BuildContext context) async {
    // Capture photo first
    final photo = await Navigator.push<File?>(
        context, MaterialPageRoute(builder: (c) => const PhotoCaptureScreen()));
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          if (context.mounted)
            ScaffoldMessenger.of(context)
                .showSnackBar(const SnackBar(content: Text('ENTRY RECORDED')));
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (approved)
            ElevatedButton(
                onPressed: () => _recordEntry(context),
