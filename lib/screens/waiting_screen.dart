...
class _WaitingScreenState extends ConsumerState<WaitingScreen> {
  ...
  void _startListening() async {
    ...
    _sub = ws.messages.listen((event) {
      ...
      final vid = event['visitorId']?.toString() ??
          event['payload']?['visitorId']?.toString();
  ...
  void _goResult(bool approved, Map<String, dynamic> event) {
    ...
    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
            builder: (c) => ApprovalResultScreen(
                approved: approved,
  ...
  Widget build(BuildContext context) {
    ...
    if (conn == 'online') {
      ...
      badgeColor = Colors.orange;
      badgeText =
          countdown > 0 ? 'Reconnecting in ${countdown}s' : 'Reconnecting';
      ...
      body: Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            if (conn == 'reconnecting' && (total ?? 0) > 0) ...[
              ...
              SizedBox(
                  ...
                  child: LinearProgressIndicator(
                      value: ((total ?? 1) - (countdown ?? 0)) / (total ?? 1),
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            ElevatedButton(
              onPressed: (conn == 'online' || conn == 'connecting')
                  ? null
                  : () {
                      try {
                        final ws = ref.read(websocketServiceProvider);
